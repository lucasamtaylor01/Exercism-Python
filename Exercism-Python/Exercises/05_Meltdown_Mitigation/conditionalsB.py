...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if product < low_threshold:
        return 'LOW'
    elif product <= high_threshold:
        return 'NORMAL'
    else:
        return 'DANGER'


def efficiency_band(voltage, current, theoretical_max_power):
    """
    Determine the efficiency band of a power generator.

    :param voltage: float - voltage value.
    :param current: float - current value.
    :param theoretical_max_power: float - theoretical maximum power.
